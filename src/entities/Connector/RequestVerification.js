...
export default function RequestUniqueConnector(props) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    isLoading(true);
    verificationChain.methods.requestUnique(account, bankerAddress, verifyAddress, signature).send({ from: account })
    .once('receipt', async (receipt) => {
        alert('Request Made to Banker!');
        console.log(receipt);
        isLoading(false);
    })
  }
    ...
    <Grid container style={{ backgroundColor: "white", display: "center", alignItems: "center", maxWidth: 400, justify: "center"}}>
        ...
        <div className={classes.paper}>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
                <TextField variant="outlined" onChange={ handleInputChange } required fullWidth  id="bankerAddress" label="Banker Address" name="bankerAddress"/>
            </Grid>
            <Grid item xs={12}>
            ...
            <Button
              bankerAddress="submit" fullWidth variant="contained" color="primary" className={classes.submit} onClick={ handleSubmit } >
              Submit
            </Button>
