...
contract VerificationChain {
    ...
    event respondEvent(address client, address indexed initiator, address bundleAddr, bytes signature, uint indexed timestamp);
    event sendEvent(address indexed initiator, address client, address bundleAddr, bytes signature, uint indexed timestamp);
    ...
    function respondToSender(address client, address initiator, address bundleAddr, bytes memory signature) external {
    ...
    function uniqueaddresslinkerHandleBundle(
        ...
        if(uniqueaddresslinkertype == 1) { 
            ...
            VerifyCO_C(cid).pickDC(_addr, msg.sender);
        }
    }
    ...
    function requestDetailAdder(address recipientAddr, address senAddr, address detailAdderAddr, bytes memory signature) external {
        emit buyEvent(recipientAddr, senAddr, detailAdderAddr, signature, now);
    }
